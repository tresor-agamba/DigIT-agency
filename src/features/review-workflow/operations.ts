...
export async function publishVersionForReview(versionId:string){return prisma.$transaction(async tx=>{
  const version=await tx.projectVersion.findUnique({where:{id:versionId},select:{id:true,projectId:true,status:true,project:{select:{status:true,clientId:true}},deliverables:{where:{OR:[{previewKind:"FILE",previewStorageKey:{not:null},previewMimeType:{not:null}},{previewKind:"EXTERNAL_URL",previewUrl:{not:null}}]},select:{id:true},take:1}}});
  ...
  await tx.project.update({where:{id:version.projectId},data:{status:"REVIEW"}});await createNotification(tx,{recipientId:version.project.clientId,type:"PREVIEW_READY",title:"Prévisualisation disponible",message:"Une nouvelle version de votre projet est prête pour validation.",link:`/client/projects/${version.projectId}/versions/${version.id}`,dedupeKey:`preview-ready:${version.id}`});return{projectId:version.projectId,versionId:version.id};
})}
...
export async function requestVersionChanges(clientId:string,versionId:string,title:string,description:string){const owned=await prisma.projectVersion.findFirst({where:{id:versionId,project:{clientId}},select:{id:true}});if(!owned)throw new ReviewWorkflowError("Version introuvable.");return prisma.$transaction(async tx=>{
  ...
  await tx.project.update({where:{id:version.projectId},data:{status:"CHANGES_REQUESTED"}});await notifyActiveAdmins(tx,{type:"MODIFICATION_REQUEST_CREATED",title:"Nouvelle demande de modification",message:"Un client a demandé des modifications sur une version.",link:`/admin/requests/${request.id}`,dedupeKey:`modification-request-created:${request.id}`});return{...request,projectId:version.projectId,versionId:version.id};
})}
...
export async function approveReviewedVersion(clientId:string,versionId:string){const owned=await prisma.projectVersion.findFirst({where:{id:versionId,project:{clientId}},select:{id:true}});if(!owned)throw new ReviewWorkflowError("Version introuvable.");try{return await prisma.$transaction(async tx=>{
  ...
  await tx.project.update({where:{id:version.projectId},data:{status:"APPROVED"}});await notifyActiveAdmins(tx,{type:"VERSION_APPROVED",title:"Version approuvée par le client",message:"Une version de projet vient d’être approuvée.",link:`/admin/projects/${version.projectId}/versions/${version.id}`,dedupeKey:`version-approved:${version.id}`});return{...approval,projectId:version.projectId,versionId:version.id};
...
export async function changeModificationRequest(requestId:string,status:"IN_PROGRESS"|"COMPLETED"|"REJECTED",adminResponse?:string){return prisma.$transaction(async tx=>{
  const request=await tx.modificationRequest.findUnique({where:{id:requestId},select:{id:true,status:true,clientId:true,version:{select:{projectId:true}}}});
  ...
  if(updated.count!==1)throw new ReviewWorkflowError("Cette demande a déjà été modifiée.");await createNotification(tx,{recipientId:request.clientId,type:"MODIFICATION_REQUEST_UPDATED",title:"Demande de modification mise à jour",message:"Le statut de votre demande de modification a changé.",link:`/client/projects/${request.version.projectId}`,dedupeKey:`modification-request-updated:${request.id}:${status}`});return{requestId:request.id,projectId:request.version.projectId};
})}
